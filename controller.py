...
# ------------------- Windows methods--------------------------
    def show_Launcher(self):
        try:
            ...
            self.Launcher.launch.clicked.connect(lambda: self.Launcher.gamestart(self.gamepath + '/RimWorldWin64.exe'))
            self.Launcher.show()
        except Exception as err:
            self.logs.error(err, exc_info=True)
...
# I want to believe that no one will break this function, because I'm too lazy to rewrite it
        ...
        try:
            ...
            if self.check == 0:
                ...
                QMessageBox.about(self, "Attention",
                                  ...
                                  (C:/Steam/steamapps/common/RimWorld as example)''')
    ...
    def get_Disk_Links(self):
        try:
            self.steam = '/Steam/steamapps/common/RimWorld/'
    ...
    def set_Game_Location_MM(self):
        try:
            self.steamMM = self.gamepath[:self.gamepath.find('steamapps') + 10] + 'workshop/content/294100/'
        except Exception:
            self.steamMM = ''
